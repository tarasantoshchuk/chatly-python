...
def home():
    if 'username' in session:
        ...
        shared_accounts = dbsession.query(Account).join(AccountMember).join(User).filter(User.username == session['username'] and AccountMember.user_id == User.id).all()
...
def add_transaction():
    if 'username' not in session:
        return 'Unauthorized', 401
    user = dbsession.query(User).filter(User.username == session['username']).first()
    transaction = Transaction(name=request.form['name'], amount=request.form['amount'], user_id=user.id, account_id=request.form['account_id'])
